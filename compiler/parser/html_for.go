...
// htmlForStatement parses a for statement within HTML content context.
// Uses the same AST node as regular for statements but uses HTML content block parsing.
func (p *Parser) htmlForStatement() (ast.Stmt, error) {
	// Check for async
	isAsync := false
	...
	// Check for else
	elseBody, err := p.htmlForElseBlock()
	if err != nil {
		return nil, err
	}
...
// htmlForElseBlock parses an else block for for statements within HTML content context.
// Returns empty slice if no else block is present.
func (p *Parser) htmlForElseBlock() ([]ast.Stmt, error) {
